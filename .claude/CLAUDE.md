...
- `lib/jekyll-aeo/generators/` — `markdown_page.rb` (per-page .md via post_write hook), `llms_txt.rb`, `llms_full_txt.rb`, `url_map.rb`, `domain_profile.rb` (all invoked from site post_write hook), `robots_txt.rb` (Jekyll::Generator — crawler policy, search vs training bots)
