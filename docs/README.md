...
jekyll_aeo:
  ...
  markdown_pages:
    ...
    html_fallback: false           # convert rendered HTML to markdown for plugin-generated pages
    html_fallback_selector: null   # CSS selector for content extraction (default: auto-detect main/article/body)
  llms_txt:
...
Pages generated by Jekyll plugins (e.g., jekyll-paginate, jekyll-archives) have no source file on disk and are normally skipped. With `html_fallback: true`, the plugin converts their rendered HTML output to markdown instead:

```yaml
jekyll_aeo:
  markdown_pages:
    html_fallback: true
    html_fallback_selector: null   # optional CSS selector
```

The converter automatically extracts the main content area (`<main>`, then `<article>`, then `<body>`) and strips layout chrome (`<script>`, `<style>`, `<nav>`, `<header>`, `<footer>`). Set `html_fallback_selector` to a CSS selector (e.g., `".content"`, `"#main"`) to target a specific element.
